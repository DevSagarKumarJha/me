...
export function DiscordModal({ isOpen, onClose, data }: DiscordModalProps) {
  ...
  const getAvatarUrl = () => {
    ...
    const defaultAvatarIndex =
      (parseInt(user.discriminator || "0000", 10) || 0) % 5;
        ...
        <div className="px-6 pb-6 relative">
          ...
          <div className="flex justify-between items-end -mt-14 mb-3">
            <div className="relative">
              ...
              <div
                className={`absolute bottom-1 right-1 w-7 h-7 rounded-full border-[5px] border-background ${getStatusColor(
                  discordStatus
                )}`}
                          ...
                          {activity.assets?.small_image && (
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <Image
                                  src={
                                    getImageUrl(activity.assets.small_image)!
                                  }
                                  alt="Small Asset"
                                  width={32}
                                  height={32}
                        ...
                        <div className="flex items-center gap-1.5 mt-2 text-xs text-muted-foreground/80 bg-muted px-2 py-1 rounded-md border border-border/50 w-fit">
                          ...
                          <span>
                            {(() => {
                              ...
                              if (diffHrs > 0)
                                return `${diffHrs}h ${diffMins % 60}m elapsed`;
